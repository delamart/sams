...
abstract class CollectionDbLib
{
    ...
    public function countAll($where = '', $order = '')
    {
        $where = $where ? sprintf('WHERE %s',$where) : '';
        $order = $order ? sprintf('ORDER BY %s',$order) : '';

        $query = sprintf('SELECT COUNT(1) FROM %s %s %s', $this->table, $where, $order);
    ...
    protected function validate_date($errors, &$values, $field)
    {
        if(isset($values[$field])) {
            $v = $values[$field];
            if(!preg_match('~^([0-9]{2,4})/([0-9]{1,2})/([0-9]{1,2})$~', $v, $matches)) {
                $errors[$field] = "Invalid date: $v should be YYYY/MM/DD";
            }
    ...
    public function delete(ModelDbLib $model) {
        ...
        if($id)
        {
            $where = sprintf('%s = "%s"',$this->pk_column, $id);
						...
						// by default SQLITE doesn't support LIMIT in DELETE,UPDATE
						$db = DbLib::getInstance();
						$limit = $db->getAttribute(PDO::ATTR_DRIVER_NAME) == 'sqlite' ? null : 1;
					
            return $this->deleteWhere($where,$limit);
        }
    }    
